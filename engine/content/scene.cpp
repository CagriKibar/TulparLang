...
void write_entity(Out &o, const SceneEntity &e) {
  ...
  if (e.components & kSceneModel) {
    if (e.primitive >= 0) {
      o.puts("  model ilkel "); o.num((float)e.primitive); o.ch(' '); o.vec(e.tint);
    } else {
      o.puts("  model kaynak "); o.num((float)e.asset); o.ch(' '); o.vec(e.tint);
    }
    if (e.metallic != 0.0f || e.roughness != 1.0f || e.reflectance != 0.5f ||
      ...
      o.ch(' '); o.vec(e.emissive); o.ch(' '); o.num(e.emissive_strength);
    }
    o.ch('\n');
  }
  ...
  if (e.components & kSceneParticle) {
    o.puts("  partikul "); o.num(e.particle_spawn_rate);
    o.puts(" omur "); o.num(e.particle_lifetime_min); o.ch(' '); o.num(e.particle_lifetime_max);
    o.puts(" boyut "); o.num(e.particle_size_start); o.ch(' '); o.num(e.particle_size_end);
    o.puts(" hiz "); o.vec(e.particle_velocity);
  ...
  if (e.components & kSceneWater) {
    ...
    o.num(e.wave_steepness); o.ch(' '); o.num(e.wave_speed); o.ch(' '); o.vec(e.wave_direction); o.ch('\n');
  }
  if (e.components & kSceneWind) {
    o.puts("  ruzgar "); o.vec(e.wind_direction); o.ch(' '); o.num(e.wind_strength); o.ch(' ');
  ...
  while (i <= len) {
    ...
    if (in_entity) {
      ...
      if (tok_is(t[0], "konum")) {
        ...
        if (n >= (uint32_t)(num_start + 4 + 6) && tok_is(t[num_start + 4], "pbr")) {
           int ps = num_start + 5;
           if (!p.num(t[ps], &cur.metallic) || !p.num(t[ps+1], &cur.roughness) || 
               !p.num(t[ps+2], &cur.reflectance) || !p.vec(t+ps+3, &cur.emissive)) return false;
           if (n >= (uint32_t)(ps + 7)) {
               if (!p.num(t[ps+6], &cur.emissive_strength)) return false;
           }
        }
        ...
        if (!p.vec(t[1], t[2], t[3], &cur.particle_velocity)) return false;
      } else if ((seen_comp & kSceneParticle) && tok_is(t[0], "dagilim")) {
        if (n != 4) return p.fail("dagilim x y z");
        if (!p.vec(t[1], t[2], t[3], &cur.particle_jitter)) return false;
      } else if (tok_is(t[0], "arazi")) {
        if (n != 9 || (seen_comp & kSceneTerrain)) return p.fail("arazi w h cell amp freq oct seed");
        ...
        if (n != 8 || (seen_comp & kSceneWater)) return p.fail("su len amp steep speed dir_x dir_y");
        ...
        if (n != 8 || (seen_comp & kSceneWind)) return p.fail("ruzgar dir_x dir_y str gust gust_f seed");
        ...
        if (n != 6 || (seen_comp & kSceneVoxel)) return p.fail("voksel sx sy sz cell");
        float sx, sy, sz;
