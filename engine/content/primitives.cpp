...
uint32_t build_primitive_meshes(renderer::Renderer &r, renderer::MeshHandle *out) {
  ...
  make(8,  renderer::Renderer::plane(v, idx));    // Duzlem
  make(10, renderer::Renderer::cube(v, idx));     // Kup
  make(11, renderer::Renderer::sphere(v, idx));   // Kure
  make(20, renderer::Renderer::capsule(v, idx));  // Kapsul
  make(21, renderer::Renderer::cylinder(v, idx)); // Silindir
  make(22, renderer::Renderer::cone(v, idx));     // Koni
  make(23, renderer::Renderer::quad(v, idx));     // Dortgen
  make(24, renderer::Renderer::torus(v, idx));    // Simit
  return built;
}
